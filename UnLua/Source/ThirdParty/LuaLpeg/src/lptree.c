...
static void correctassociativity (TTree *tree) {
  TTree *t1 = sib1(tree);
  assert(tree->tag == luaTChoice || tree->tag == luaTSeq);
...
static void finalfix (lua_State *L, int postable, TTree *g, TTree *t) {
 tailcall:
  switch (t->tag) {
    case luaTGrammar:  /* subgrammars were already fixed */
      return;
    case luaTOpenCall: {
...
static void correctkeys (TTree *tree, int n) {
 ...
 tailcall:
  switch (tree->tag) {
    case luaTOpenCall: case luaTCall: case luaTRunTime: case luaTRule: {
      if (tree->key > 0)
        tree->key += n;
      break;
    }
...
static TTree *seqaux (TTree *tree, TTree *sib, int sibsize) {
  tree->tag = luaTSeq; tree->u.ps = sibsize + 1;
...
static TTree *numtree (lua_State *L, int n) {
  if (n == 0)
    ...
    fillseq(nd, luaTAny, n, NULL);  /* sequence of 'n' any's */
    return tree;
  }
}
...
static TTree *getpatt (lua_State *L, int idx, int *len) {
  ...
  switch (lua_type(L, idx)) {
    case LUA_TSTRING: {
      ...
      if (slen == 0)  /* empty? */
        tree = newleaf(L, luaTTrue);  /* always match */
      else {
        tree = newtree(L, 2 * (slen - 1) + 1);
        fillseq(tree, luaTChar, slen, s);  /* sequence of 'slen' chars */
      }
      break;
    }
    case LUA_TNUMBER: {
    ...
    case LUA_TBOOLEAN: {
      tree = (lua_toboolean(L, idx) ? newleaf(L, luaTTrue) : newleaf(L, luaTFalse));
      break;
    }
    case LUA_TTABLE: {
    ...
    case LUA_TFUNCTION: {
      tree = newtree(L, 2);
      tree->tag = luaTRunTime;
      tree->key = addtonewktable(L, 0, idx);
      sib1(tree)->tag = luaTTrue;
      break;
    }
    default: {
...
static int lp_seq (lua_State *L) {
  ...
  if (tree1->tag == luaTFalse || tree2->tag == luaTTrue)
    lua_pushvalue(L, 1);  /* false . x == false, x . true = x */
  else if (tree1->tag == luaTTrue)
    lua_pushvalue(L, 2);  /* true . x = x */
  else
    newroot2sib(L, luaTSeq);
  return 1;
}
...
static int lp_choice (lua_State *L) {
  ...
  else if (nofail(t1) || t2->tag == luaTFalse)
    lua_pushvalue(L, 1);  /* true / x => true, x / false => x */
  else if (t1->tag == luaTFalse)
    lua_pushvalue(L, 2);  /* false / x => x */
  else
    newroot2sib(L, luaTChoice);
  return 1;
}
...
static int lp_star (lua_State *L) {
  ...
  if (n >= 0) {  /* seq tree1 (seq tree1 ... (seq tree1 (rep tree1))) */
    ...
    for (; n > 1; n--) {  /* repeat (n - 1) times */
      tree->tag = luaTChoice; tree->u.ps = n * (size1 + 3) - 2;
      sib2(tree)->tag = luaTTrue;
      tree = sib1(tree);
      tree = seqaux(tree, tree1, size1);
    }
    tree->tag = luaTChoice; tree->u.ps = size1 + 1;
    sib2(tree)->tag = luaTTrue;
    memcpy(sib1(tree), tree1, size1 * sizeof(TTree));
  }
  copyktable(L, 1);
...
static int lp_sub (lua_State *L) {
  ...
  if (tocharset(t1, &st1) && tocharset(t2, &st2)) {
    ...
    tree->tag = luaTSeq;  /* sequence of... */
    tree->u.ps =  2 + s2;
    sib1(tree)->tag = luaTNot;  /* ...not... */
...
static int lp_V (lua_State *L) {
  TTree *tree = newleaf(L, luaTOpenCall);
...
static int capture_aux (lua_State *L, int cap, int labelidx) {
  TTree *tree = newroot1sib(L, luaTCapture);
  tree->cap = cap;
...
static int lp_matchtime (lua_State *L) {
  TTree *tree;
  luaL_checktype(L, 2, LUA_TFUNCTION);
  tree = newroot1sib(L, luaTRunTime);
...
static void buildgrammar (lua_State *L, TTree *grammar, int frule, int n) {
  ...
  nd->tag = luaTTrue;  /* finish list of rules */
}
...
static int checkloops (TTree *tree) {
 tailcall:
  if (tree->tag == luaTRep && nullable(sib1(tree)))
    return 1;
  else if (tree->tag == luaTGrammar)
...
static int verifyrule (lua_State *L, TTree *tree, int *passed, int npassed,
 ...
 tailcall:
  switch (tree->tag) {
    ...
    case luaTTrue:
    case luaTBehind:  /* look-behind cannot have calls */
      return 1;
    case luaTNot: case luaTAnd: case luaTRep:
    ...
    case luaTCall:
      ...
      tree = sib2(tree); goto tailcall;
    case luaTSeq:  /* only check 2nd child if first is nb */
      if (!verifyrule(L, sib1(tree), passed, npassed, 0))
        return nb;
      /* else return verifyrule(L, sib2(tree), passed, npassed, nb); */
      tree = sib2(tree); goto tailcall;
    case luaTChoice:  /* must check both children */
