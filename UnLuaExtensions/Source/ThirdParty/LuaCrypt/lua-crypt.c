...
static const uint32_t g_r[] = {7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22, 7, 12, 17, 22,
...
static void
push64(lua_State *L, uint64_t _r)
{
    uint8_t tmp[8];
    tmp[0] = _r & 0xff;
    tmp[1] = (_r >> 8) & 0xff;
...
luaopen_crypt(lua_State *L)
{
    ...
    if (!init)
    {
        ...
        init = 1;
#ifdef __MSVC
        srand((rand() << 8) ^ (time(NULL) << 16) ^ _getpid());
#else
