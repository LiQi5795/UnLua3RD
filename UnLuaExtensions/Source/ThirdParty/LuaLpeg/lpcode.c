...
int tocharset (TTree *tree, Charset *cs) {
  switch (tree->tag) {
    case luaTSet: {  /* copy set */
      loopset(i, cs->cs[i] = treebuffer(tree)[i]);
      return 1;
    }
    case luaTChar: {  /* only one char */
...
static int callrecursive (TTree *tree, int f (TTree *t), int def) {
  int key = tree->key;
  assert(tree->tag == luaTCall);
  assert(sib2(tree)->tag == luaTRule);
...
int hascaptures (TTree *tree) {
 tailcall:
  switch (tree->tag) {
    case luaTCapture: case luaTRunTime:
      return 1;
    case luaTCall:
      return callrecursive(tree, hascaptures, 0);
    case luaTRule:  /* do not follow siblings */
      tree = sib1(tree); goto tailcall;
    case luaTOpenCall: assert(0);
    default: {
...
int checkaux (TTree *tree, int pred) {
 tailcall:
  switch (tree->tag) {
    case luaTChar: case luaTSet: case luaTAny:
    case luaTFalse: case luaTOpenCall:
      return 0;  /* not nullable */
    case luaTRep: case luaTTrue:
      return 1;  /* no fail */
    case luaTNot: case luaTBehind:  /* can match empty, but can fail */
      if (pred == PEnofail) return 0;
      else return 1;  /* PEnullable */
    case luaTAnd:  /* can match empty; fail iff body does */
      if (pred == PEnullable) return 1;
      /* else return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case luaTRunTime:  /* can fail; match empty iff body does */
      if (pred == PEnofail) return 0;
      /* else return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case luaTSeq:
    ...
    case luaTChoice:
      ...
      /* else return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case luaTCapture: case luaTGrammar: case luaTRule:
      /* return checkaux(sib1(tree), pred); */
      tree = sib1(tree); goto tailcall;
    case luaTCall:  /* return checkaux(sib2(tree), pred); */
      tree = sib2(tree); goto tailcall;
...
int fixedlen (TTree *tree) {
 ...
 tailcall:
  switch (tree->tag) {
    case luaTChar: case luaTSet: case luaTAny:
      return len + 1;
    case luaTFalse: case luaTTrue: case luaTNot: case luaTAnd: case luaTBehind:
      return len;
    case luaTRep: case luaTRunTime: case luaTOpenCall:
      return -1;
    case luaTCapture: case luaTRule: case luaTGrammar:
...
static int getfirst (TTree *tree, const Charset *follow, Charset *firstset) {
 tailcall:
  switch (tree->tag) {
    ...
    case luaTRunTime: {  /* function invalidates any follow info. */
    ...
    case luaTBehind: {  /* instruction gives no new information */
...
static int headfail (TTree *tree) {
 tailcall:
  switch (tree->tag) {
    case luaTChar: case luaTSet: case luaTAny: case luaTFalse:
      return 1;
    case luaTTrue: case luaTRep: case luaTRunTime: case luaTNot:
    case luaTBehind:
      return 0;
    case luaTCapture: case luaTGrammar: case luaTRule: case luaTAnd:
...
static int needfollow (TTree *tree) {
 tailcall:
  switch (tree->tag) {
    case luaTChar: case luaTSet: case luaTAny:
    case luaTFalse: case luaTTrue: case luaTAnd: case luaTNot:
    case luaTRunTime: case luaTGrammar: case luaTCall: case luaTBehind:
      return 0;
    case luaTChoice: case luaTRep:
      return 1;
    case luaTCapture:
      tree = sib1(tree); goto tailcall;
    case luaTSeq:
...
static void codegrammar (CompileState *compst, TTree *grammar) {
  ...
  for (rule = sib1(grammar); rule->tag == luaTRule; rule = sib2(rule)) {
    positions[rulenumber++] = gethere(compst);  /* save rule position */
    codegen(compst, sib1(rule), 0, NOINST, fullset);  /* code rule */
    addinstruction(compst, IRet, 0);
  }
  assert(rule->tag == luaTTrue);
...
static void codegen (CompileState *compst, TTree *tree, int opt, int tt,
 ...
 tailcall:
  switch (tree->tag) {
    case luaTChar: codechar(compst, tree->u.n, tt); break;
    case luaTAny: addinstruction(compst, IAny, 0); break;
    case luaTSet: codecharset(compst, treebuffer(tree), tt); break;
    case luaTTrue: break;
    case luaTFalse: addinstruction(compst, IFail, 0); break;
    case luaTChoice: codechoice(compst, sib1(tree), sib2(tree), opt, fl); break;
    case luaTRep: coderep(compst, sib1(tree), opt, fl); break;
    case luaTBehind: codebehind(compst, tree); break;
    case luaTNot: codenot(compst, sib1(tree)); break;
    case luaTAnd: codeand(compst, sib1(tree), tt); break;
    case luaTCapture: codecapture(compst, tree, tt, fl); break;
    case luaTRunTime: coderuntime(compst, tree, tt); break;
    case luaTGrammar: codegrammar(compst, tree); break;
    case luaTCall: codecall(compst, tree); break;
    case luaTSeq: {
